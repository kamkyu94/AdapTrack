...
class MotChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            'GT_FOLDER': os.path.join(code_path, 'gt/mot_challenge/'),  # Location of GT data
            'TRACKERS_FOLDER': os.path.join(code_path, 'trackers_legacy/mot_challenge/'),  # Trackers location
            'OUTPUT_FOLDER': None,  # Where to save eval results (if None, same as TRACKERS_FOLDER)
            'TRACKERS_TO_EVAL': None,  # Filenames of trackers_legacy to eval (if None, all in folder)
            'CLASSES_TO_EVAL': ['pedestrian'],  # Valid: ['pedestrian']
            'BENCHMARK': 'MOT17',  # Valid: 'MOT17', 'MOT16', 'MOT20', 'MOT15'
            'SPLIT_TO_EVAL': 'train',  # Valid: 'train', 'test', 'all'
            'INPUT_AS_ZIP': False,  # Whether trackers input files are zipped
            ...
            'TRACKER_DISPLAY_NAMES': None,  # Names of trackers_legacy to display, if None: TRACKERS_TO_EVAL
            'SEQMAP_FOLDER': None,  # Where seqmaps are found (if None, GT_FOLDER/seqmaps)
            'SEQMAP_FILE': 'None',  # Directly specify seqmap file (if none use seqmap_folder/benchmark-split_to_eval)
            'SEQ_INFO': None,  # If not None, directly specify sequences to eval and their number of timesteps
            'GT_LOC_FORMAT': '{gt_folder}/{seq}/gt/gt.txt',  # '{gt_folder}/{seq}/gt/gt.txt'
            'SKIP_SPLIT_FOL': False,  # If False, data is in GT_FOLDER/BENCHMARK-SPLIT_TO_EVAL/ and in
                                      # TRACKERS_FOLDER/BENCHMARK-SPLIT_TO_EVAL/trackers/
    ...
    def __init__(self, config=None):
        ...
        if self.config['TRACKER_DISPLAY_NAMES'] is None:
            ...
            raise TrackEvalException('List of trackers files and trackers display names do not match.')
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        """Load a file (gt or trackers) in the MOT Challenge 2D box format
            ...
            if time_key in read_data.keys():
                try:
                    ...
                    if is_gt:
                        ...
                        raise TrackEvalException(
                            'Cannot convert tracking data from trackers %s, sequence %s to float. Is data corrupted?' % (
                                tracker, seq))
                try:
                    ...
                    if is_gt:
                        ...
                        err = 'Cannot load trackers data from trackers %s, sequence %s, because there is not enough ' \
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        Notes:
            General preprocessing (preproc) occurs in 4 steps. Some datasets may not use all of these steps.
                ...
                2) Match gt dets and trackers dets. Remove trackers dets that are matched to a gt det that is of a
                    distractor class, or otherwise marked as to be removed.
                3) Remove unmatched trackers dets if they fall within a crowd ignore region or don't meet a certain
                    other criteria (e.g. are too small).
                4) Remove gt dets that were only useful for preprocessing and not for actual evaluation.
            After the above preprocessing steps, this function also calculates the number of gt and trackers detections
                and unique track ids. It also relabels gt and trackers ids to be contiguous and checks that ids are
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            # Match trackers and gt dets (with hungarian algorithm) and remove trackers dets which match with gt dets
            ...
            # Apply preprocessing to remove all unwanted trackers dets.
