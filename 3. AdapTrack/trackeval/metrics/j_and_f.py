...
class JAndF(_BaseMetric):
    ...
    def eval_sequence(self, data):
        ...
        if frame_shape:
            ...
            # also perform zero padding if number of trackers IDs < number of ground truth IDs
    ...
    def _compute_f(gt_data, tracker_data, tracker_data_id, gt_id, bound_th):
        """
        Perform F computation for a given gt and a given trackers ID. Adapted from
        https://github.com/davisvideochallenge/davis2017-evaluation
        :param gt_data: the encoded gt masks
        :param tracker_data: the encoded trackers masks
        :param tracker_data_id: the trackers ID
        :param gt_id: the ground truth ID
        :param bound_th: boundary threshold parameter
        :return: the F value for the given trackers and gt ID
        """
    ...
    def _compute_j(gt_data, tracker_data, num_gt_ids, num_tracker_ids, num_timesteps):
        """
        Computation of J value for all ground truth IDs and all trackers IDs in the given sequence. Adapted from
        https://github.com/davisvideochallenge/davis2017-evaluation
        :param gt_data: the ground truth masks
        :param tracker_data: the trackers masks
        :param num_gt_ids: the number of ground truth IDs
        :param num_tracker_ids: the number of trackers IDs
