...
class Evaluator:
    ...
    def evaluate(self, dataset_list, metrics_list):
        ...
        for dataset, dataset_name in zip(dataset_list, dataset_names):
            ...
            # print('\nEvaluating %i trackers(s) on %i sequence(s) for %i class(es) on %s dataset using the following '
            #       'metrics: %s\n' % (len(tracker_list), len(seq_list), len(class_list), dataset_name,
            #                          ', '.join(metric_names)))

            # Evaluate each trackers
            for tracker in tracker_list:
                # if not config['BREAK_ON_ERROR'] then go to next trackers without breaking
                try:
