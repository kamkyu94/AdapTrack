...
def plot_compare_trackers(tracker_folder, tracker_list, cls, output_folder, plots_list=None):
    """Create plots which compare metrics across different trackers_legacy."""
    # Define what to plot
...
def load_multiple_tracker_summaries(tracker_folder, tracker_list, cls):
    """Loads summary data for multiple trackers_legacy."""
    data = {}
...
def create_comparison_plot(data, out_loc, y_label, x_label, sort_label, bg_label=None, bg_function=None, settings=None):
    """ Creates a scatter plot comparing multiple trackers_legacy between two metric fields, with one on the x-axis and the
    ...
    print('\nPlotting %s vs %s, for the following (ordered) trackers_legacy:' % (y_label, x_label))
