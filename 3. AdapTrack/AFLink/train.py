...
def train(save: bool):
    ...
    if save:
        if not exists(cfg.model_save_dir): os.mkdir(cfg.model_save_dir)
        torch.save(model.state_dict(), join(cfg.model_save_dir, 'newmodel_epoch{}_tmp.pth'.format(epoch + 1)))
